...
export function Configuracoes() {
  const fields=[
    {label:'Escritório',value:'Dr. Rafael Costa — Advogado',icon:'🏛'},
    {label:'Número WhatsApp Business',value:'(11) 9 9999-0001',icon:'📱'},
    {label:'E-mail para notificações',value:'rafael@costadv.com.br',icon:'📧'},
    {label:'Plano atual',value:'Básico — 1 número · até 500 atendimentos/mês',icon:'📋'},
  ]
  const notifs=[
    {label:'Novo lead recebido',on:true},
    {label:'IA não conseguiu responder',on:true},
    {label:'Conversa aguardando há mais de 1h',on:true},
    {label:'Relatório semanal por e-mail',on:false},
  ]
  const [nState, setNState] = useState(notifs.map(n=>n.on))

  return (
    <div className="p-5 max-w-md">
      <PageHeader title="Configurações"/>
      {fields.map(f=>(
        <div key={f.label} className="card mb-3 flex items-center gap-3">
          <span className="text-xl flex-shrink-0">{f.icon}</span>
          <div className="flex-1 min-w-0">
            <p className="text-[11px] text-gray-400 mb-0.5">{f.label}</p>
            <p className="text-sm font-medium text-gray-900">{f.value}</p>
          </div>
          <button onClick={()=>alert(`Editar: ${f.label}`)} className="text-gray-300 hover:text-gray-500">✏️</button>
        </div>
      ))}
      <div className="card mb-4">
        <p className="font-medium text-gray-900 mb-3">🔔 Notificações</p>
        {notifs.map((n,i)=>(
          <div key={n.label} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
            ...
            <Toggle on={nState[i]} onChange={v=>setNState(p=>p.map((s,j)=>j===i?v:s))}/>
          </div>
        ))}
      </div>
      <button onClick={()=>alert('Configurações salvas!')} className="btn-gold w-full justify-center py-2.5">Salvar</button>
    </div>
  )
}
