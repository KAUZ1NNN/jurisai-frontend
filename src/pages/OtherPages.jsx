...
import { Search, Plus, UserPlus, Bot, BookOpen, AlertTriangle, Clock, Check,
  IdCard, FileText, Coins, Gavel, MessageCircle, ClipboardList, Star,
  RefreshCw, FileWarning, Zap, BarChart2, Loader2 } from 'lucide-react'
import { api } from '../lib/api'
import { KANBAN_STAGES } from '../lib/mock'
  ...
  return (
    <div className="p-5">
      <PageHeader title="Clientes" actions={<>
        ...
        <button className="btn-gold" onClick={() => alert('Em breve: formulário de novo cliente')}>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {sc.length === 0 && (
                    <div className="text-center py-6">
                      <UserPlus size={20} className="text-gray-300 mx-auto mb-1"/>
...
export function IAConfig() {
  ...
  useEffect(() => {
    ...
    if (!BASE) { setWelcome(''); setLoading(false); return }
    fetch(`${BASE}/settings`, { headers:{ Authorization:`Bearer ${token}` } })
      .then(r => r.json())
      .then(s => setWelcome(s.welcome_message ?? ''))
      .catch(() => setWelcome(''))
      .finally(() => setLoading(false))
  }, [])
  ...
  async function saveWelcome() {
    ...
    if (BASE) {
      await fetch(`${BASE}/settings`, {
        ...
        body: JSON.stringify({ welcome_message: welcome }),
      })
    }
    setSaving(false)
    setEditW(false)
    ...
    <div className="p-5 max-w-xl">
      ...
      <div className="flex items-start gap-2.5 p-3 rounded-xl mb-4 border"
        ...
        <p className="text-sm text-gray-600">
          <span className="font-medium text-gray-900">Modo: Envio automático ativo. </span>
          A IA responde e envia todas as mensagens. Exceções abaixo.
        </p>
      </div>
      ...
      <div className="card mb-3">
        ...
        {[
          ...
          ['bancario',       'Direito Bancário',         'Tarifas indevidas, juros abusivos, superendividamento'],
        ].map(([k,l,d]) => (
          <div key={k} className="flex items-start gap-2 p-2.5 rounded-lg border mb-2 last:mb-0"
            ...
            <div className="w-4 h-4 rounded bg-[#C8A84B] flex items-center justify-center flex-shrink-0 mt-0.5">
              <Check size={11} color="#081724"/>
            </div>
            <div>
      ...
      <div className="card mb-3">
        ...
        {[
          {k:'cpf',      l:'CPF, RG, dados bancários ou senhas',   I:IdCard},
          {k:'docs',     l:'Solicitação de documentos jurídicos',   I:FileText},
          {k:'values',   l:'Valores em propostas de honorários',    I:Coins},
          {k:'lawsuits', l:'Processos judiciais específicos',       I:Gavel},
        ].map(({k,l,I}) => (
      ...
      {/* Horário — lê de settings */}
      <div className="card mb-3">
        <div className="flex items-center gap-2 mb-2"><Clock size={15} style={{color:'#C8A84B'}}/><span className="font-medium">Horário de funcionamento</span></div>
        <p className="text-xs text-gray-400">Configure em <strong>Configurações</strong> → Horário de funcionamento da IA.</p>
      </div>

      {/* Boas-vindas — lê do banco */}
      <div className="card mb-4">
        <div className="flex items-center gap-2 mb-3"><span>👋</span><span className="font-medium">Mensagem de boas-vindas</span></div>
        {loading ? (
          <div className="flex items-center gap-2 text-gray-400 text-sm py-2"><Loader2 size={14} className="animate-spin"/> Carregando...</div>
        ) : editW ? (
          <textarea rows={4} value={welcome} onChange={e => setWelcome(e.target.value)}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#C8A84B] resize-none mb-2"/>
        ) : (
          <div className="bg-gray-50 rounded-lg px-3 py-2.5 text-sm text-gray-800 leading-relaxed mb-2">
            {welcome || <span className="text-gray-400 italic">Nenhuma mensagem configurada</span>}
          </div>
        )}
        <div className="flex gap-2">
          {editW ? (
            <>
              <button onClick={saveWelcome} disabled={saving} className="btn-gold text-xs">
                {saving ? <><Loader2 size={12} className="animate-spin"/> Salvando...</> : 'Salvar'}
              </button>
              <button onClick={() => setEditW(false)} className="btn-ghost text-xs">Cancelar</button>
            </>
          ) : (
            <button onClick={() => setEditW(true)} className="btn-ghost text-xs">Editar mensagem</button>
          )}
        </div>
      </div>

      <button onClick={() => alert('Tom e gatilhos salvos!')} className="btn-gold w-full justify-center py-2.5">
...
export function Automacoes() {
  ...
  function toggle(id) {
    setAutos(p => p.map(a => a.id === id ? {...a, on:!a.on} : a))
    api.automations.toggle(id)
  }

  return (
    <div className="p-5 max-w-2xl">
      <PageHeader title="Automações" subtitle="Disparos automáticos por área — sem intervenção humana"
        actions={<button className="btn-gold" onClick={() => alert('Em breve: editor de automação')}><Zap size={14}/> Nova automação</button>}/>
      {loading ? (
        <div className="flex items-center gap-2 text-gray-400 py-8 justify-center">
              ...
              <div key={a.id} className="card flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-[11px] px-2 py-0.5 rounded-lg"
                    style={{background: a.on ? '#E1F5EE' : '#f3f4f6', color: a.on ? '#085041' : '#9ca3af'}}>
                    {a.stat}
                  </span>
                </div>
...
export function Relatorios() {
  ...
  // Sem dados ainda
  if (!stats || stats.activeConversations === 0 && stats.totalClients === 0) return (
    <div className="p-5 max-w-3xl">
      <PageHeader title="Relatórios"
        actions={<div className="flex gap-1.5">
          {['7 dias','30 dias','3 meses'].map((p,i) => (
            <button key={p} onClick={() => setPeriod(['7d','30d','3m'][i])}
              className="text-xs px-3 py-1.5 rounded-lg cursor-pointer border-0"
              style={{background: period === ['7d','30d','3m'][i] ? '#C8A84B' : '#f3f4f6',
                      color:      period === ['7d','30d','3m'][i] ? '#081724' : '#6b7280'}}>
              {p}
            </button>
          ))}
        </div>}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
        ...
        <p className="font-medium text-gray-500 mb-1">Nenhum dado ainda</p>
        <p className="text-sm text-gray-400">Os relatórios aparecerão aqui assim que houver conversas e atendimentos registrados no sistema.</p>
      </div>
    </div>
  )

  const weekly = stats.weeklyConversations ?? [0,0,0,0,0,0,0]
  ...
  return (
    <div className="p-5 max-w-3xl">
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        <KpiCard label="Conversas totais"  value={stats.totalConversations ?? 0} sub="No período"          icon={MessageCircle} iconColor="#378ADD"/>
        <KpiCard label="Enviadas pela IA"  value={`${stats.aiRate ?? 0}%`}       sub="Sem intervenção"     icon={Bot}           iconColor="#1D9E75"/>
        <KpiCard label="Revisões geradas"  value={stats.totalAlerts ?? 0}        sub="Sensíveis / docs"    icon={AlertTriangle} iconColor="#E24B4A"/>
        <KpiCard label="Clientes ativos"   value={stats.totalClients ?? 0}       sub="Cadastrados"         icon={Star}          iconColor="#C8A84B"/>
      </div>

      {/* Gráfico de barras */}
      <div className="card mb-4">
        <p className="font-medium text-gray-900 mb-4">Conversas por dia (últimos 7 dias)</p>
        {weekly.every(v => v === 0) ? (
          <div className="text-center py-6 text-gray-400 text-sm">Nenhuma conversa no período</div>
        ) : (
          <div className="flex items-end gap-2" style={{height:100}}>
            {days.map((d,i) => (
              <div key={d} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[11px] text-gray-400">{weekly[i]}</span>
                <div className="w-full rounded-t transition-all"
                  style={{height: Math.round(weekly[i]/max*72), background: i===3 ? '#C8A84B' : 'rgba(200,168,75,0.3)'}}/>
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Por área */}
        <div className="card">
          <p className="font-medium text-gray-900 mb-3">Atendimentos por área</p>
          {!stats.areaBreakdown?.length ? (
            <p className="text-sm text-gray-400 text-center py-4">Sem dados ainda</p>
          ) : (
            stats.areaBreakdown.map(({area, pct, color}) => (
        ...
        <div className="card">
          ...
          {!stats.funnel?.length ? (
            ...
            stats.funnel.map(({label, value, pct}) => (
